-- notification_logs is expected to be one of the highest-volume tables as the
-- product grows; it previously had no business_id column at all, forcing
-- tenant-scoped queries through a nullable open_slot_id join, and no index on
-- customer_id/created_at. Add the column, backfill it, index it, and update
-- the one RPC insert site so new rows are populated going forward.

alter table public.notification_logs
  add column if not exists business_id uuid references public.businesses (id) on delete cascade;

update public.notification_logs nl
set business_id = os.business_id
from public.open_slots os
where nl.open_slot_id = os.id
  and nl.business_id is null;
...
-- Re-define staff_send_open_slot_offers so its notification_logs insert also
-- stamps business_id (it already has p_business_id in scope via its lock on
-- open_slots). Signature is unchanged, so existing grants still apply.
